...
function PresetButton($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="card p-4"><div class="flex items-center gap-3 mb-3"><span class="text-2xl">${escape_html(icon(name))}</span> <div><h3 class="font-medium text-content-primary">${escape_html(preset.name)}</h3> <p class="text-xs text-content-secondary">${escape_html(preset.power ? `${preset.brightness}% / ${preset.colorTemp}K` : "Off")}</p></div></div> <button class="w-full py-2 rounded-lg bg-accent hover:bg-accent/80 text-white font-medium relative transition-all">Apply to All Lamps `);
    {
      $$renderer2.push("<!--[!-->");
    }
